...
const fraud = global.client = new Client({ fetchAllMembers: true });;
const config = require("./config.json");
const fs = require("fs");
fraud.commands = new Collection();
fraud.aliases = new Collection();

fs.readdirSync("./commands").filter(file => file.endsWith(".js")).forEach(file => {
    let command = require(`./commands/${file}`);
    fraud.commands.set(command.conf.command, command);
    console.log(`[Command] ${file.replace(".js", "")} command loaded.`);
    command.conf.aliases.forEach(aliases => {
        fraud.aliases.set(aliases, command)  
    });
});

fs.readdirSync("./events").filter(file => file.endsWith(".js")).forEach(file => {
    let event = require(`./events/${file}`);
    fraud.on(event.conf.event, event.execute);
    console.log(`[Event] ${file.replace(".js", "")} event loaded.`);
});

  function guvenli(kisiID) {
    let uye = fraud.guilds.cache.get(config.guildID).members.cache.get(kisiID);
    let fraudsafe = config.whitelist || [];
    if (!uye || uye.id === fraud.user.id || uye.id === config.OwnerID || uye.id === uye.guild.owner.id || fraudsafe.some(fraud => uye.id === fraud || uye.roles.cache.has(fraud))) return true
  else return false};
  ...
  function cezalandir(kisiID, tur) {
    let userID = fraud.guilds.cache.get(config.guildID).members.cache.get(kisiID);
    if (!userID) return;
    ...
    if (tur == "ban") return userID.ban({ reason: "Fraud Koruma Sistemi" }).catch()};

fraud.on('ready', async () => {
  fraud.user.setActivity(`${config.Status}`, { status: "online"} ,{ type: 'PLAYİNG'})
  .then(console.log(`${fraud.user.tag} İsmiyle Discord Bağlantısı kuruldu.`))
  .catch(() => console.log(`Bir hata ile karşılaştım.`))
});

//-                                                                        ROL KORUMA                                                                        -\\

fraud.on("roleCreate", async role => {
  ...
  role.delete({ reason: "Fraud Koruma Sistemi" });
  cezalandir(entry.executor.id, "jail");
  let user = fraud.users.cache.get(entry.executor.id)
  let logKanali = fraud.channels.cache.get(config.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setThumbnail(user.displayAvatarURL({dynamic:true})).setColor("BLUE").setTitle('Sunucuda izinsiz bir rol oluşturuldu!').setDescription(`${entry.executor} adlı yetkili tarafından sunucuda izinsiz bir rol oluşturuldu! \n\nSunucuda rolü oluşturan yetkilinin rolleri alındı ve cezalı rolü verildi!`).setFooter(`Fraud Koruma Sistemi`).setTimestamp())}});

fraud.on("roleDelete", async role => {
